...
protected trait BlogIntents {
  ...
  object ConfiguredSeg {
    ...
    def apply(path: List[String]) = new ConfiguredSegImpl { val configPath = path }
  ...
  def insertPost[A](mongo:Mongo)(p: A)(implicit mongoRecord: MongoRecord[A], ev: BSONDocumentWriter[A]) = {
    ...
    (for {
      ...
      flatMapK(f => Try(Await.result(f, 1000 millis)) match { // flatMap inside the monad
        case scala.util.Success(s) => \/-(s)
        case scala.util.Failure(e) => -\/(e)
      }).
      mapK[ResultM, LastError] { // Map again the monad to get a ResultM again
      case \/-(s) => Result.Success(s)
      case -\/(e) => e match {
